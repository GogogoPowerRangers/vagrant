...
HOSTNAME=$(hostname | sed -e 's#\..*##'); export HOSTNAME
if [ ! "$1" = "" ] ; then
    SYSTEM=$1
elif [ "$SYSTEM" = "" ] ; then
...
if [ "$SYSTEM" = "$HOSTNAME" -a -d $CANDLEHOME/logs ] ; then
    ...
    cp -p $CANDLEHOME/logs/kasmain* $UNIT_DIR 2> /dev/null
    cp -p $CANDLEHOME/logs/*_as_*.log $UNIT_DIR
fi
echo "Copy files to $UNIT_DIR"
for f in ${SYSTEM}*/*
do
