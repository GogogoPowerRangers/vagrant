...
installIT() {
    ...
    rpm -Uhv smai-oslc-pm-06.35.00.00-1.el6.x86_64.rpm || exit 1
        
    # Wait for ITMcDP component running on server1 server to report it has started successfully
    CHECK_STATUS="/bin/grep -F \"CWWKZ0001I: Application ITMREST started\" /opt/ibm/wlp/usr/servers/server1/logs/messages.log"
    eval "$CHECK_STATUS" > /dev/null 2>&1
    while [ $? -ne 0 ]; do
        /bin/sleep 1
        eval "$CHECK_STATUS" > /dev/null 2>&1
    done
    
    # Install application support files for OSLC, ITMcDP and APM UI
    cd support    
    rpm -Uhv smai-*            
    cd ../..
        
    cp oslcConfigs.txt ${CCM_DIR}/oslc_pm/config/oslcConfigs.txt
    ${CCM_DIR}/oslc_pm/bin/itmcmd config -p ${CCM_DIR}/oslc_pm/config/oslcConfigs.txt -A as
    ${CCM_DIR}/oslc_pm/bin/itmcmd agent start as  
}
